...
export const FieldStateProvider = ({ children }) => {
    ...
    const fetchData = async () => {
        try {
            ...
            const [pulseRes, historyRes, yieldRes] = await Promise.all([
                axios.get(`${API_URL}/field-logs/latest`),
                axios.get(`${API_URL}/field-logs/history`),
                axios.get(`${API_URL}/yield-analytics`)
            ]);
            ...
            // Fetch Prediction Separately from Backend Proxy (avoids CORS)
            setPredictionLoading(true);
            try {
                const predictRes = await axios.post(`${API_URL}/predict`, defaultInput);
            ...
            console.warn("Backend unavailable, using mock data for PoC");

            // FALLBACK MOCK DATA for PoC robustness
